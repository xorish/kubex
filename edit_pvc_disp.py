...
class Ui_MainWindow(object):
    ...
    def save_pvc(self):
        ...
        file = open('temp_pvc.yaml', 'w')
        file.write(text)
        file.close()
        file = open('temp_pvc.yaml', 'r')
        yaml_data = yaml.safe_load(file)
        file.close()
        del yaml_data["status"]
        file = open('temp_pvc.json', 'w')
        json.dump(yaml_data,file,indent=6)
        file.close()
        time.sleep(1)
        self.process_edit_pvc_consent = QtCore.QProcess()
        self.process_edit_pvc_consent.start('python3', ['-u', f'{path}/edit_pvc_consent.py'])
...
"")
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", f"PVC ({pvc})"))
