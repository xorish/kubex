...
class Ui_MainWindow(object):
    ...
    def yesx(self):
        ...
        kube_cmd=f'/home/{user}/bin/kubectl apply -f ~/temp_pv.json'
        ssh_cmd_file=f"sshpass -p {password} scp -o StrictHostKeyChecking=no {path}/temp_pv.json  {user}@{ip}:~/ "
        ssh_cmd_kube=f"sshpass -p {password} ssh -o StrictHostKeyChecking=no {user}@{ip} '{kube_cmd}' "
        ssh_cmd_rm = f"sshpass -p {password} ssh -o StrictHostKeyChecking=no {user}@{ip} 'rm ~/temp_pv.json' "
...
"font: 11pt \"Ubuntu\";")
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", f"PV-Update ({pv})"))
        self.yes.setText(_translate("MainWindow", "Yes"))
        self.no.setText(_translate("MainWindow", "No"))
        self.label.setText(_translate("MainWindow", f"Are you sure to update PV\n'{pv}' ?"))
def main():
