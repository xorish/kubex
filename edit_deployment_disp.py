...
class Ui_MainWindow(object):
    ...
    def save_deployment(self):
        ...
        file = open('temp_deployment.yaml', 'w')
        file.write(text)
        file.close()
        file = open('temp_deployment.yaml', 'r')
        yaml_data = yaml.safe_load(file)
        file.close()
        del yaml_data["status"]
        print(yaml_data)
        file = open('temp_deployment.json', 'w')
        json.dump(yaml_data,file,indent=6)
        file.close()
        time.sleep(1)
        self.process_edit_deployment_consent = QtCore.QProcess()
        self.process_edit_deployment_consent.start('python3', ['-u', f'{path}/edit_deployment_consent.py'])
...
"")
        ...
        self.callProgram()
        self.save_edit.clicked.connect(lambda : self.save_deployment())

    def retranslateUi(self, MainWindow):
        _translate = QtCore.QCoreApplication.translate
        MainWindow.setWindowTitle(_translate("MainWindow", f"Deployment ({deployment})"))
